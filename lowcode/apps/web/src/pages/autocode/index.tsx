...
export default function AutocodePage() {
  ...
  const startGenerateJob = async (dto: AutoCodeDto) => {
    ...
    if (dto.packageId) {
      const pkg = packageOptions.find((p) => p.id === dto.packageId);
      if (pkg) {
        const pkgKebab = pkg.name.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || pkg.name;
        modulePath = `/pkg/${pkgKebab}/${kebab}`;
      }
    }
