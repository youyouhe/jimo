...
export class AccountService {
  ...
  async findAll(query: QueryAccountDto, userId?: string, isAdmin: boolean = false): Promise<PaginatedData<Accounts>> {
    const { page, pageSize, code, name, account_type, balance_direction } = query;
    const offset = (page - 1) * pageSize;

    const conditions: SQL[] = [isNull(accounts.deletedAt)];
    const _ownership = this.ownershipHelper.visibleCondition(accounts.ownerId, accounts.sharedWith, userId, isAdmin, 'public');
    ...
    if (balance_direction) {
      conditions.push(eq(accounts.balance_direction, balance_direction));
    }
    ...
    const [rows, totalRows] = await Promise.all([
      ...
      parent_id_display: parent_alias.name,
        ...
        .from(accounts)
        .leftJoin(parent_alias, eq(accounts.parent_id, parent_alias.id))
        .where(whereClause)
  ...
  async create(dto: CreateAccountDto, userId?: string): Promise<Accounts> {
    ...
    const existingByName = await this.db
      .select()
      .from(accounts)
      .where(and(eq(accounts.name, dto.name), isNull(accounts.deletedAt)))
      .limit(1);
